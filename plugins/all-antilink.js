...
const isLinkBatu = /🗿/i // tambahin sendiri
...
export async function before(m, { conn, args, usedPrefix, command, isAdmin, isBotAdmin }) {
	...
	const isAntiLinkIg = isLinkBatu.exec(m.text)
	...
	if (!isAdmin) {
		if (chat.antiLinkTik && isAntiLinkTik) {
			...
			if (isBotAdmin) await conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: hapus }})
		} else if (chat.antiLinkBatu && isAntiLinkBatu) {
			await conn.sendButton(m.chat, bwah, author, ['off antibatu🗿', '/disable antibatu🗿'], m)
			if (isBotAdmin) await conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: hapus }})
